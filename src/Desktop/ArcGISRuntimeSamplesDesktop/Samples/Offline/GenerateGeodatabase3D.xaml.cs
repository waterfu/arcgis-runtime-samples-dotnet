...
namespace ArcGISRuntime.Samples.Desktop
{
    ...
    /// </summary>
    /// <title>Generate Geodatabase 3D</title>
    /// <category>Offline</category>
    public partial class GenerateGeodatabase3D : UserControl
    {
		...
		{
			var extentWebMercator = new Envelope(-13644594, 4538221, -13614635, 4552889, SpatialReferences.WebMercator);
			var extentWGS84 = GeometryEngine.Project(extentWebMercator, SpatialReferences.Wgs84);
        ...
        private async void GenerateGeodatabaseButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
				...
				MySceneView.Scene.Layers["wildfireGroup"].IsVisible = false;
            }
            catch (Exception ex)
            {
        ...
        private async Task CreateFeatureLayersAsync(string gdbPath)
        {
            try
            {
				...
				var groupLayer = MySceneView.Scene.Layers["Local_Geodatabase"] as GroupLayer;
                if (groupLayer != null)
					MySceneView.Scene.Layers.Remove(groupLayer);
				...
				MySceneView.Scene.Layers.Add(groupLayer);

				await MySceneView.SetViewAsync(new Viewpoint(extent.Expand(1.10)));
            }
            catch (Exception ex)
            {

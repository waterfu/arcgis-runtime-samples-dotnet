...
namespace ArcGISRuntime.Samples.Desktop.T
{
    /// <summary>
    /// This sample demonstrates how to synchronize a local geodatabase with an online service.
    /// </summary>
    /// <title>Sync Geodatabase 3D</title>
    /// <category>Offline</category>
	/// <sampleType>Workflow</sampleType>
    public partial class SyncGeodatabase3D : UserControl, INotifyPropertyChanged
    {
		...
		public SyncGeodatabase3D()
        {
            InitializeComponent();

            _syncTask = new GeodatabaseSyncTask(new Uri(BASE_URL));
			_onlineBirdsLayer = MySceneView.Scene.Layers.OfType<ArcGISDynamicMapServiceLayer>().First();
        ...
        private async void GenerateButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Generate local gdb
				await GenerateLocalGeodatabaseAsync(MySceneView.GetCurrentViewpoint(ViewpointType.BoundingGeometry).TargetGeometry as Envelope);
        ...
        private async void UnregisterButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (LocalBirdsLayer != null)
                {
					...
					MySceneView.Scene.Layers.Remove(LocalBirdsLayer);
        ...
        private async void RegisterBirdSightingButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                bird.Geometry = new MapPoint(TapLocation.X, TapLocation.Y, TapLocation.SpatialReference);
        ...
        private async Task CreateFeatureLayers(Geodatabase gdb)
        {
			...
			MySceneView.Scene.Layers.Insert(2, LocalBirdsLayer);

            await MySceneView.SetViewAsync(new Viewpoint(birdsTable.Extent));
        }
        ...
        private async Task RefreshDataView()
        {
            ...
            Query query = new Query("1=1") { Geometry = (MySceneView.GetCurrentViewpoint(ViewpointType.BoundingGeometry).TargetGeometry as Envelope), OutFields = new OutFields(new string[] { "globalid" }) };

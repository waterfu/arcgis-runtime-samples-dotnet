...
{
    /// <summary>
    /// Sample shows how to create a SimpleRenderer for a graphics layer in XAML and code. 
	/// User added points will be added to the graphics layer and rendered using the GraphicsLayer Renderer.
    /// </summary>
    /// <title>Simple Renderer 3D</title>
	/// <category>Symbology</category>
	public partial class SimpleRendererSample3D : UserControl
    {
        private Random _random = new Random();
		private GraphicsOverlay _graphicsOverlay;

        /// <summary>Construct Simple Renderer sample control</summary>
		public SimpleRendererSample3D()
        {
            InitializeComponent();
		...
		private async void MySceneView_SpatialReferenceChanged(object sender, System.EventArgs e)
		{
			...
			{
				MessageBox.Show("Error loading states data: " + ex.Message, "Simple Renderer Sample 3D");
			}
		}
        ...
        private async Task AcceptPointsAsync()
        {
            try
            {
				while (MySceneView.GetCurrentViewpoint(ViewpointType.BoundingGeometry).TargetGeometry.Extent != null)
                {

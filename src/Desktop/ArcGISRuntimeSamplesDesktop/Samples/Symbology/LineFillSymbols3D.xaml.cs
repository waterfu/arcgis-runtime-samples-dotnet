...
namespace ArcGISRuntime.Samples.Desktop
{
	...
	/// </summary>
	/// <title>Line and Fill Symbols 3D</title>
	/// <category>Symbology</category>
	public partial class LineFillSymbols3D : UserControl
	{
		private List<SampleSymbol> _symbols;
		private GraphicsOverlay _graphicsOverlay;

		/// <summary>Construct Line and Fill Symbols sample control</summary>
		public LineFillSymbols3D()
		{
			InitializeComponent();
		...
		private async void MySceneView_SpatialReferenceChanged(object sender, System.EventArgs e)
		{
			...
			{
				MessageBox.Show("Error loading states data: " + ex.Message, "Line Fill Symbols 3D");
			}
		}
			...
			try
			{
				while (MySceneView.GetCurrentViewpoint(ViewpointType.BoundingGeometry).TargetGeometry.Extent != null)
				{
					...
					if (sampleSymbol.Symbol is LineSymbol)
						shape = await MySceneView.Editor.RequestShapeAsync(DrawShape.Polyline, sampleSymbol.Symbol);
					else
						shape = await MySceneView.Editor.RequestShapeAsync(DrawShape.Polygon, sampleSymbol.Symbol);
